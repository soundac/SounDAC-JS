...
var prepareWifOrPassword = function(userName, passwordOrWif, requiredKeyType /*owner, active, basic, memo*/)
{
...
var readAuthorityPubKey = function(authority)
{
  return authority && authority.key_auths && authority.key_auths.length > 0 ? authority.key_auths[0][0] : null;
};

var vestingMuse = function(account, gprops) {
    const vests = parseFloat(account.vesting_shares.split( ' ' )[0]);
    const total_vests = parseFloat(gprops.total_vesting_shares.split( ' ' )[0]);
    const total_vest_muse = parseFloat(gprops.total_vesting_fund_muse.split( ' ' )[0]);
    const vesting_musef = total_vest_muse * (vests / total_vests);
      ...
        },
        {
          ...
          else
          {
            ...
            var fctSuccess = function() {
              callback(0, "Success");
            };
            var fctFunds = function()
            {
    ...
    api.getBalanceObjects(addressesToUse, function(err, result) {
      ...
      if(result && result.length > 0)
      {
        result.forEach(function(aBalance)
        {
          ...
          broadcast.balanceClaim(active, targetAccount, aBalance.id, pubKey.toString(), aBalance.balance, function(err, result) {
            facade.lastError = err;
            if(result)
            {
